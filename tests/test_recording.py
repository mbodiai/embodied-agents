...
def recorder(tempdir):
    ...
    observation_space = spaces.Dict(
        {"image": Image(size=(224, 224)).space(), "instruction": spaces.Discrete(10)})
...
def test_record(tempdir, recorder):
    observation = {"image": np.ones(
        (224, 224, 3), dtype=np.uint8), "instruction": 0}
    action = {"gripper_position": np.zeros(
        (3,), dtype=np.float32), "gripper_action": 1}
    ...
    with h5py.File(recorder.filename, "r") as f:
        ...
        assert np.array_equal(f["action/gripper_position"]
                              [0], action["gripper_position"])
        assert f["action/gripper_action"][0] == action["gripper_action"]


def test_record_image_class(tempdir, recorder):
    observation = {"image": Image(
        np.ones((224, 224, 3), dtype=np.uint8)), "instruction": 0}
    action = {"gripper_position": np.zeros(
        (3,), dtype=np.float32), "gripper_action": 1}
    recorder.record(observation, action)
    ...
    with h5py.File(recorder.filename, "r") as f:
        ...
        assert np.array_equal(f["observation/image"][0],
