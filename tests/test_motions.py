...
def test_pose6d_deserialization():
    ...
    assert (pose.x, pose.y, pose.z, pose.roll, pose.pitch,
            pose.yaw) == (1, 0.9, 0.9, 0.1, 0.2, 0.3)
...
def test_mobile_single_arm_control_serialization():
    msac = MobileSingleArmControl(
        ...
        head=HeadControl(tilt=JointControl(value=1.0),
                         pan=JointControl(value=-1.0))
    )
...
def test_hand_control_serialization():
    hc = HandControl(
        pose=Pose6D(x=0.5, y=-0.5, z=0.5, roll=0.5, pitch=-0.5, yaw=0.5),
        grasp=JointControl(value=1.0)
    )
...
def test_hand_control_deserialization():
    json_data = '{"pose": {"x": 0.5, "y": -0.5, "z": 0.5, "roll": 0.5, "pitch": -0.5, "yaw": 0.5}, "grasp": {"value": 1.0}}'
