...
from mbodied_agents.agents.sense.utils.detect_segment_utils import (
    DetectionResult,
    get_boxes,
    plot_detections,
    refine_masks,
...
class ObjectDetector2D:
    ...
    def detect(self, image: Image.Image, labels: List[str] = None, threshold: float = 0.3) -> List[Dict[str, Any]]:
    ...
    def segment(
        self, image: Image.Image, detection_results: List[Dict[str, Any]], polygon_refinement: bool = False
    ) -> List[DetectionResult]:
        """Segments objects in the given image based on detection results.

        Args:
            image (Image.Image): The input image for object segmentation.
    ...
    def detect_and_segment(
        ...
        Args:
            image (Image.Image): The input image for detection and segmentation.
